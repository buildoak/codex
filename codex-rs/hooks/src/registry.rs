...
pub struct HooksConfig {
    ...
    pub before_tool_use_argv: Option<Vec<String>>,
...
impl Hooks {
    pub fn new(config: HooksConfig) -> Self {
        let after_agent = config
            ...
            .into_iter()
            .collect();
        let before_tool_use = config
            .before_tool_use_argv
            .filter(|argv| !argv.is_empty() && !argv[0].is_empty())
            .map(|argv| crate::command_hook(argv, "before_tool_use".to_string()))
            .into_iter()
            .collect();
        let after_tool_use = config
            .after_tool_use_argv
            .filter(|argv| !argv.is_empty() && !argv[0].is_empty())
            .map(|argv| crate::after_tool_use_command_hook(argv, "after_tool_use".to_string()))
            .into_iter()
            .collect();
